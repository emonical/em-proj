...
@pytest.mark.skip(reason="session stop full implementation deferred to Plan 11-02")
def test_daemon_stop_when_not_running(clean_db: redis_module.Redis) -> None:
    """Placeholder: stop with no running daemon exits gracefully.

    Full implementation requires Plan 11-02 (session stop verb reads daemon
    HASH, validates pid via is_holder_stale, sends SIGTERM and waits).
    This stub ensures test collection succeeds and documents the expected
    behaviour for Plan 11-02.
    """
    result = _run_cli(["session", "stop"])
